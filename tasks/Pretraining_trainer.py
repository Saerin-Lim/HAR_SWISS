...
class SWISS_trainer(object):
    ...
    def signal_level_loss_fn(self, x, y):
        # x : B, S, T
    ...
    def run(self, epochs):
        loss_dict = {'total_loss':[],'feature_level_loss':[],'signal_level_loss':[]}
        ...
        for epoch in range(1, epochs+1):
            ...
            epoch_history = {
                'Loss': {
                    ...
                    'signal_level_loss': train_history.get('signal_level_loss')
                    }
                }
            ...
            make_description( ['Total Loss', 'Feature Level', 'Signal Level'],
                             round(train_history["total_loss"],4), 
                             round(train_history["feature_level_loss"],4),
                             round(train_history["signal_level_loss"],4))
    ...
    def train(self, pretrain_loader, current_epoch: int):
        ...
        feature_level_loss, signal_level_loss, total_scalar_loss = .0, .0, .0
        ...
        for i, (crt_x, _, masks, _) in enumerate(pretrain_loader):
            ...
            signal_level_loss = self.signal_level_loss_fn(recon_out, x)
            signal_level_loss += signal_level_loss.item()
            
            total_loss = self.auto_loss(feature_level_loss, signal_level_loss)
